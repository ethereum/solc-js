      ...
      expect(() => {
        ...
        test('file import without lazy-loading callback fails properly (using lowlevel API)', () => {
          ...
          for (let error in output.errors) {
            ...
            expect((output.errors[error].indexOf('Error') !== -1 && output.errors[error].indexOf('File import callback not supported') !== -1)
            || output.errors[error].indexOf('not found: File import callback not supported') !== -1).toBeTruthy();
          }
        });
    ...
    test("compiling standard JSON (using libraries)", () => {
      ...
      expect(L.length).toBeGreaterThan(0);
        ...
        test('compiling standard JSON (with warning >=0.4.0)', () => {
          ...
          let output = JSON.parse(solc.compile(JSON.stringify(input)));
          expect(expectError(output, 'Warning', 'Source file does not specify required compiler version!')).toBeTruthy();
        });
    ...
    test("compiling standard JSON (using libraries) (using lowlevel API)", () => {
      ...
      expect(L.length).toBeGreaterThan(0);
        ...
        test('compiling standard JSON (invalid JSON)', () => {
          ...
          expect(expectError(output, 'JSONError', 'Line 1, Column 2\n  Missing \'}\' or object member name') || expectError(output, 'JSONError', 'Invalid JSON supplied:')).toBeTruthy();
        });
  
        test('compiling standard JSON (invalid language)', () => {
          let output = JSON.parse(solc.compile('{"language":"InvalidSolidity","sources":{"cont.sol":{"content":""}}}'));
          expect(expectError(output, 'JSONError', 'supported as a language.') && expectError(output, 'JSONError', '"Solidity"')).toBeTruthy();
        });
  
        test('compiling standard JSON (no sources)', () => {
          let output = JSON.parse(solc.compile('{"language":"Solidity"}'));
          expect(expectError(output, 'JSONError', 'No input sources specified.')).toBeTruthy();
        });
  
        test('compiling standard JSON (multiple sources on old compiler)', () => {
          let output = JSON.parse(solc.compile('{"language":"Solidity","sources":{"cont.sol":{"content":"import \\"lib.sol\\";"},"lib.sol":{"content":""}}}'));
          if (solc.features.multipleInputs) {
            expect(expectNoError(output)).toBeTruthy();
          } else {
            expect(expectError(output, 'JSONError', 'Multiple sources provided, but compiler only supports single input.') || expectError(output, 'Parser error', 'Parser error: Source not found.')).toBeTruthy();
          }
        });
      });
